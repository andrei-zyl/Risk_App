...
namespace Risk_App
{
    public partial class Graph_Unit : Window
    {
        public Graph_Unit() { InitializeComponent(); }
        public Graph_Unit(ChartValues<decimal> bbb, List<string> ccc, ChartValues<decimal> ddd, DateTime dp_begin_gr, DateTime dp_end_gr) : this()
        {
            this.bbb = bbb;
            this.ccc = ccc;
            ...
            gr_label.Text = "Структура прямых и потенциальных потерь от операционных инцидентов в разрезе подразделений с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            filename = "Структура потерь в разрезе подразделений с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            DataContext = this;
        }
        string filename;
