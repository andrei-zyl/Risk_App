...
namespace Risk_App
{
    public partial class Chart_Loss : Window
    {
        public Chart_Loss()
        {
            InitializeComponent();
        }
        public Chart_Loss(ChartValues<decimal> bbb, List<string> ccc, DateTime dp_begin_gr, DateTime dp_end_gr) : this()
        {
            this.bbb = bbb;
            this.ccc = ccc;
            int j = 0;
            ...
            gr_label.Text = "Структура операционных инцидентов в разрезе типов потерь с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            filename = "Структура инцидентов в разрезе типов потерь с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            DataContext = this;
        }
