...
namespace Risk_App
{
    public partial class Graph_Loss : Window
    {
        public Graph_Loss() { InitializeComponent(); }
        public Graph_Loss(ChartValues<decimal> bbb, List<string> ccc, ChartValues<decimal> ddd, DateTime dp_begin_gr, DateTime dp_end_gr) : this()
        {
            this.bbb = bbb;
            this.ccc = ccc;
            ...
            gr_label.Text = "Структура прямых и потенциальных потерь от операционных инцидентов в разрезе типов потерь с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            filename = "Структура потерь в разрезе типов потерь с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            DataContext = this;
        }
        string filename;
