...
namespace Risk_App
{
    public partial class Chart_SR : Window
    {
        public Chart_SR()
        {
            InitializeComponent();
        }
        public Chart_SR(ChartValues<decimal> bbb, List<string> ccc, DateTime dp_begin_gr, DateTime dp_end_gr) : this()
        {
            this.bbb = bbb;
            this.ccc = ccc;
            int j = 0;
            ...
            gr_label.Text = "Структура операционных инцидентов в разрезе источников риска с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            filename = "Структура инцидентов в разрезе источников риска с " + dp_begin_gr.ToString("dd.MM.yyyy") + " по " + dp_end_gr.ToString("dd.MM.yyyy");
            DataContext = this;
        }
